...
parameterised_models = [example_models.construct_four_state_chain(),
                        example_models.construct_mazhari_chain(),
                        example_models.construct_wang_chain(),
                        example_models.construct_kemp_model()]

current_file = os.path.basename(__file__)
output_dir = os.environ.get('MARKOVBUILDER_TEST_OUTPUT', os.path.join(
    'test_output', current_file))
...
def test_transition_matrix():
    ...
    mc.draw_graph(os.path.join(output_dir, "BeattieModel.html"), show_options=True)
    mc.draw_graph(os.path.join(output_dir, "BeattieModel.html"))
    ...
    assert pen_and_paper_B == system[1]
...
@pytest.mark.parametrize("mc", models)
def test_construct_examples(mc):
    """ Output the example models as pyvis files and DOT files """
    name = mc.name
    mc.draw_graph(os.path.join(output_dir, "%s_graph.html" % name))
...
def test_parameterise_rates_no_default():
...
@pytest.mark.parametrize("mc", parameterised_models)
def test_myokit_output(mc):
    """
...
def test_construct_open_trapping_model():
    """
    ...
    for mc in models:
        ...
        # Save dotfile
        nx.drawing.nx_agraph.write_dot(mc.graph, os.path.join(output_dir, "open_trapping.dot"))

        mc.draw_graph(os.path.join(output_dir, "%s_open_trapping.html" % mc.name))
        logging.debug(mc.graph)


def test_assert_reversibility_using_cycles():
...
def test_equate_rates():
    ...
    mc.draw_graph(os.path.join(output_dir, '%s_rates_substitution.html' % mc.name), show_rates=True)
    label_found = False
    for _, _, d in mc.graph.edges(data=True):
        if 'label' in d:
            label_found = True
            if d['label'] == 'k1':
                assert rates_dict['k1'] == d['rate']
    assert label_found
    ...
    assert 'd_k_2*d_k_4' in transition_rates
    assert 'd_k_2' in mc.rates
    assert 'd_k_4' in mc.rates
...
@pytest.mark.parametrize("mc", parameterised_models)
def test_sample_trajectories(mc):
    ...
    with pytest.raises(ValueError):
        mc.get_equilibrium_distribution()
    ...
    fig.savefig(os.path.join(output_dir, 'beattie_model_sample_trajectories'),
                transparent=True)
    fig.clf()
...
def test_HH_models(model_size):
    ...
    mc.draw_graph(os.path.join(output_dir, mc.name + ".html"))
    # update one rate
